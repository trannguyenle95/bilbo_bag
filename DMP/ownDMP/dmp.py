...
from ownDMP.cs import CanonicalSystem
...
class DMPs(object):
    ...
    def imitate_path(self, y_des, plot=False):
        ...
        for d in range(self.n_dmps):
            print("self.goal[d] - self.y0[d]:", self.goal[d] - self.y0[d])

            #k = self.goal[d] - self.y0[d] #NOTE: I MOVED DIVISION HERE FROM "gen_weights"!!
            ...
            K = self.ay * self.by
            D = self.ay
            x_track = self.cs.rollout() #NOTE: also changed "gen_front_term()" to only return x, as it is used by "gen_weights" at end of this method!!
            f_target[:, d] = (ddy_des[d] + D[d]*dy_des[d])/K[d] - (self.goal[d] - y_des[d]) + (self.goal[d] - self.y0[d])*x_track
            

        # efficiently generate weights to realize f_target
        self.gen_weights(f_target) # NOTE: GEN_WEIGHTS INCLUDES SPATIAL SCALNG TERM BY DEFAULT - I REMOVED IT SO IT COULD BE ADDED ABOVE!!!!!
        ...
        if plot is True:
            ...
            plt.plot(psi_track)
            #plt.title("basis functions", psi_track)
    ...
    def step(self, tau=1.0, error=0.0, external_force=None):
        ...
        for d in range(self.n_dmps):
            ...
            # DMP acceleration
            #self.ddy[d] = (
            #    self.ay[d] * (self.by[d] * (self.goal[d] - self.y[d]) - self.dy[d]) + f * (self.goal[d] - self.y0[d])
            #)

            #NOTE: Changed to match notation without (g-y0):
            K = self.ay * self.by
            D = self.ay
            self.ddy[d] = (
                K[d]*(self.goal[d] - self.y[d]) - D[d]*self.dy[d] - K[d]*(self.goal[d] - self.y0[d])*x + K[d]*f
            )
