...
class DMPs_discrete(DMPs):
    ...
    def gen_weights(self, f_target):
        ...
        #print("x track shape:", x_track.shape)
        #print("psi_track:", psi_track)
        #print("psi track shape:", psi_track.shape)
        # efficiently calculate BF weights using weighted linear regression
        self.w = np.zeros((self.n_dmps, self.n_bfs))
        for d in range(self.n_dmps):
            # spatial scaling term
            #k = self.goal[d] - self.y0[d] #NOTE: I MOVED DIVISION TO "imitate_path"!!
