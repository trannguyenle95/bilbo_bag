...
int loadJointAnglesFromFile(std::string jointAnglesFile, bool areJointAnglesInDegree, std::array<double, 7> &jointAngles){
    // Load Desired Joint Position
    std::ifstream fileJointAngles;
    fileJointAngles.open(jointAnglesFile.c_str());
    if(fileJointAngles){
        for(int i = 0; i < 7; ++i){
            fileJointAngles >> jointAngles[i];
			if(areJointAnglesInDegree){
				jointAngles[i] = RAD(jointAngles[i]);
			}
        }
        fileJointAngles.close();
    }
    else{
        std::cout << " File " << jointAnglesFile << " not found!!!" << std::endl;
        return -1;
    }
	return 0;
}
