...
{   
    ...
    this->_robot->setCartesianImpedance({{3000, 3000, 3000, 250, 250, 250}});
...
{   
    ...
    geometry_msgs::Quaternion ori_d_msg= msg->pose.pose.orientation;
    double duration = msg->duration;
    double pose_x = pose.x;
    double pose_y = pose.y;
    double pose_z = pose.z;
    double ori_x = ori_d_msg.x;
    double ori_y = ori_d_msg.y;
    double ori_z = ori_d_msg.z;
    double ori_w = ori_d_msg.w;
    bool enable = msg->enable;
...
//     std::cout << "Trajectory length=" << N << ", end points=" << Nend << std::endl;
    ...
    if (enable)
    {
        ...
        // Eigen::Matrix3d R_end = ori_d.toRotationMatrix();
        // std::cout << "R_end=" << std::endl << R_end << std::endl;
		Tend = T0;
		Tend(1, 4) = pose_x;
		Tend(2, 4) = pose_y;
		Tend(3, 4) = pose_z;
        // test
        // Tend(1, 1) = R_end(0, 0);    Tend(1, 2) =R_end(0, 1);    Tend(1, 3) =R_end(0, 2) ;    
		// Tend(2, 1) = R_end(1, 0);    Tend(2, 2) =R_end(1, 1) ;    Tend(2, 3) =R_end(1, 2) ;    
		// Tend(3, 1) = R_end(2, 0);    Tend(3, 2) =R_end(2, 1) ;    Tend(3, 3) =R_end(2, 2) ; 
	    ...
	    Tmiddle = T0;
        Tmiddle(1, 1) = Tend(1,1);   Tmiddle(1, 2) = Tend(1,2);  Tmiddle(1, 3) = Tend(1,3);
        Tmiddle(2, 1) = Tend(2,1);   Tmiddle(2, 2) = Tend(2,2);  Tmiddle(2, 3) = Tend(2,3);
        Tmiddle(3, 1) = Tend(3,1);   Tmiddle(3, 2) = Tend(3,2);  Tmiddle(3, 3) = Tend(3,3); 
        std::cout << "Tmiddle=" << std::endl << Tmiddle << std::endl;
...
void CartesianPythonController::runControl(math::Transform3D* trajectory, int N)
{
    ...
    const double translational_stiffness{3000.0};
